package dominando.android.coroutinesample
...
class MainActivity : AppCompatActivity(), CoroutineScope {
    ...
    suspend fun tarefa1(): String {
        Log.d("FLMWG", "tarefa1 -> INICIO ${Thread.currentThread().name}")
        delay(5000)
        Log.d("FLMWG", "tarefa1 -> FIM")
        return "Tarefa1"
    }

    suspend fun tarefa2(): String {
        Log.d("FLMWG", "tarefa2 -> INICIO ${Thread.currentThread().name}")
        delay(3000)
        Log.d("FLMWG", "tarefa2 -> FIM")
        return "Tarefa2"
    }
