...
class ProviderRepository(val ctx: Context) : HotelRepository {
    ...
    override fun save(hotel: Hotel) {
        val uri = ctx.contentResolver.insert(HotelProvider.CONTENT_URI, getValues(hotel))
    ...
    override fun remove(vararg hotels: Hotel) {
        hotels.forEach { hotel ->
            val uri = Uri.withAppendedPath(HotelProvider.CONTENT_URI, hotel.id.toString())
            ctx.contentResolver.delete(uri, null, null)
        }
    }

    override fun hotelById(id: Long, callback: (Hotel?) -> Unit) {
        val cursor = ctx.contentResolver.query(Uri.withAppendedPath(HotelProvider.CONTENT_URI, id.toString()),
            null, null, null, null)
