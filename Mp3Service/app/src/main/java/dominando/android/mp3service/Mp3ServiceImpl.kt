...
//    Implementação da interface Mp3Service
    ...
    override val currentSong: String? get() = currentFile

    override val totalTime: Int get() {
        return if (mediaPlayer.isPlaying || isPaused) {
                mediaPlayer.duration
            } else 0
    }

    override val elapsedTime: Int get() {
        return if (mediaPlayer.isPlaying || isPaused) {
    ...
    private fun createNotification() {
        ...
        val pitActivity = PendingIntent.getActivity(this, 0,
                Intent(this, Mp3Activity::class.java).apply { addFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP) },
        ...
        views.setTextViewText(R.id.txtMusic, currentFile?.substring((currentFile?.lastIndexOf(File.separator) ?: 0) + 1))
