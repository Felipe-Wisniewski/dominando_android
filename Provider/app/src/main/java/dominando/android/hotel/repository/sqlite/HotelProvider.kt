...
class HotelProvider : ContentProvider() {
    ...
    override fun getType(uri: Uri): String? {
        ...
        return when (uriType) {
            TYPE_HOTEL_DIR -> return "${ContentResolver.CURSOR_DIR_BASE_TYPE}/dominando.android.hotel"
            TYPE_HOTEL_ITEM -> return  "${ContentResolver.CURSOR_ITEM_BASE_TYPE}/dominando.android.hotel"
            else -> null
        }
    }
    ...
    override fun insert(uri: Uri, values: ContentValues?): Uri? {
        ...
        when (uriType) {
            TYPE_HOTEL_DIR -> id = sqlDB.insertWithOnConflict(
                TABLE_HOTEL,
                null,
                values,
    ...
    override fun query(uri: Uri, projection: Array<String>?, selection: String?,
        ...
        when (uriType) {
            TYPE_HOTEL_DIR -> cursor = queryBuilder.query(db, projection, selection, selectionArgs,
                null, null,sortOrder)

            TYPE_HOTEL_ITEM -> {
                queryBuilder.appendWhere("$COLUMN_ID = ?")
                cursor = queryBuilder.query(db, projection, selection, arrayOf(uri.lastPathSegment),
                    null, null, null)
            }
            else -> throw IllegalArgumentException("URI não suportada: $uri")
        }
