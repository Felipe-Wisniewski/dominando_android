...
class ContactFragment : DialogFragment(), DialogInterface.OnClickListener {
    ...
    override fun onClick(dialog: DialogInterface?, which: Int) {
        ...
        if (edtName.text.isNotEmpty() && edtPhone.text.isNotEmpty() && photo != null) {
            ...
            ContactUtils.insertContact(requireContext(), edtName.text.toString(),
                edtPhone.text.toString(), edtAddress.text.toString(), photo)
        ...
        if (resultCode == Activity.RESULT_OK && data != null && data.data != null) {
            try {
                ...
                selectedPhoto = BitmapFactory.decodeStream(
                    requireContext().contentResolver.openInputStream(data.data), null, options)
