...
class RoomRepositoryTests {
    ...
    @Before fun before_each_test() {
        ...
        dummyHotel = Hotel(id = 0, name = "Hotel Recife", address = "Av. Boa Viagem", rating = 4.5f)
    }
    ...
    @Test fun update_all_hotel_fields() {
        ...
        val updateHotel = newHotel()
        updateHotel.id = hotelId
        repo.save(updateHotel)

        val loadedHotel = LiveDataTestUtil.getValue(repo.hotelById(hotelId))
        assertEquals(updateHotel, loadedHotel)
    }

    @Test fun load_all_inserted_hotels() {
        val allHotels = listOf(newHotel(), newHotel(), newHotel())
        allHotels.forEach { repo.save(it) }
    ...
    private fun newHotel() = Hotel(name = UUID.randomUUID().toString(),
        address = UUID.randomUUID().toString(), rating = (Math.random() * 5.0).toFloat())
}