...
class HotelHttp(private val service: HotelHttpApi, private val repository: HotelRepository,
                private val pictureFinder: FindHotelPicture, private val auth: Auth) {

    private val currentUser: String get() = auth.getUserId() ?: ""
    ...
    private fun uploadHotelPhoto(hotel: Hotel) {
        if (hotel.photoUrl.isNotEmpty() && hotel.photoUrl.startsWith("content:")) {
            ...
            when (execution) {
                is UploadResult -> {
                    Log.d("FLMWG", "Upload efetuado com sucesso")
                }

                is NoUploadPerformed -> {
                    Log.d("FLMWG", "Erro ao efetuar upload")
                }
            }
        }
    }
    ...
    fun sendRegistrationToken(token: String): Boolean {
        return currentUser.isNotBlank() &&
                token.isNotBlank() &&
                service.sendRegistrationId(currentUser, token).execute().isSuccessful
    }

    companion object {
        const val BASE_URL = "http://192.168.0.15/hotel_service/"
    }
}