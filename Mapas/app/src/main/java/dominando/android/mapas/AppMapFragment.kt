...
//            isMyLocationEnabled = true     adiciona o botão para levar a localização atual
        ...
        viewModel.getCurrentLocation().observe(this, Observer { currentLocation ->
            if (currentLocation != null) {
                ...
                if (markerCurrentLocation == null) {
                    val icon = BitmapDescriptorFactory.fromResource(R.drawable.img_se_liga)
    ...
    private fun onMapLongClick(latLng: LatLng) {
        val pit = PendingIntent.getBroadcast(requireContext(),
            0,
    ...
    private fun updateMap(mapState: MapViewModel.MapState) {
        ...
        googleMap?.run {
            ...
            if (origin != null) {
                addMarker(MarkerOptions().position(origin).title(getString(R.string.map_marker_origin)))
            ...
            if (destination != null) {
                addMarker(MarkerOptions().position(destination).title(getString(R.string.map_marker_destination)))
